...
**Background:** the observed Sentinel-1 flood (9.94% of edges) disconnects more of the population (68.6%, corrected figure — see §7c for the pathing-bug fix; this was originally reported as 94.0% before the fix) than the broader hazard-zone proxy (17.3% of edges, 51.6% disconnected, originally 74.0%). Largest surviving component (a separate, unaffected-by-that-bug fragmentation statistic) is 9.06% of nodes. The manuscript's original Finding 3 explained the disconnection gap as evidence of a network-topology mechanism — that real floods preferentially concentrate on structurally important "chokepoint" edges (river crossings), so fewer affected edges can still cause more damage than a larger, more diffusely-distributed set. Sections 3 and 5 of this document had already audited part of that claim (bridge tags, edge-overlap sensitivity) and found it held up. This section reports a more direct test of the mechanism itself, and it does not hold up.
...
**A genuine measurement discrepancy surfaced while building this, and it turned out to be a real bug, not just a caveat — see §7c for the fix.** Using this component-complete method, the *real* Sentinel-1 flood disconnects 68.65% of population from all facilities — not the 94.0% originally reported as the headline "severe" scenario figure. Both numbers were computed correctly for what they each measured; they answered different questions. The original headline pipeline (`src/06`, `src/11`) ran its multi-source Dijkstra restricted to the single **largest** post-disruption connected component only — so a population pocket stranded in a *smaller* fragment that happened to retain its own local puskesmas was counted as "no access", even though it had a real, working facility nearby. Null A2's component-complete method credited that local access, which is why its 68.65% is the *correct* figure, not merely an alternative one. §7c fixes the headline pipeline to match.

## 7c. The pathing bug was fixed, and the core Finding 3 comparison re-tested from scratch

Restricting Dijkstra to the largest component wasn't a defensible simplification — `networkx.multi_source_dijkstra_path_length` already handles disconnected graphs correctly without it. A node unreachable from every facility source is simply absent from the result; a node sharing *any* component with a facility gets a correct within-component distance, regardless of whether that component is the largest one. The restriction was removed from `src/06_flood_disruption.py` and `src/11_flood_disruption_sentinel1.py`, and every severe scenario was rerun.

**Corrected severe-scenario figures:**

| | Edges affected | Population disconnected (corrected) | Population disconnected (original, buggy) |
|---|---|---|---|
| Proxy | 17.3% | **51.6%** | 74.0% |
| Sentinel-1 (observed) | 9.94% | **68.6%** | 94.0% |

The corrected Sentinel-1 figure (68.6%) matches Null A2's independently-computed 68.65% almost exactly — strong internal consistency evidence that the fix is correct, since the two were computed by different code paths (one via component membership, one via actual Dijkstra distances) arriving at the same answer.

**The core empirical claim behind Finding 3 survives, at corrected magnitude: 68.6% > 51.6%.** Sentinel-1 still disconnects more of the population than the proxy despite affecting roughly half as many edges. This is the "it survives" outcome, not "it shrinks to nothing" or "it reverses" — both of which were live possibilities going into this check and would have been reported as such if that's what the numbers showed. The moderate scenarios, the matched-penalty sweep, leave-one-out, and the workforce-denominator check were never affected by this bug in the first place: a pure-penalty scenario removes no edges, so the graph stays a single connected component by construction, and the largest-component restriction was a no-op for every moderate run.

**What changed and what didn't, stated plainly:**
- Changed: the absolute severe-scenario disconnection percentages (74.0%→51.6% proxy, 94.0%→68.6% Sentinel-1), and everywhere in the manuscript quoting them.
- Unchanged: the retraction of the chokepoint mechanism (§7, §7b — that conclusion did not depend on the buggy pipeline, since both null models used the corrected component-membership logic from the start). The direction of the core proxy-vs-observed comparison. Every moderate-scenario, inequality, and capacity-stratification result.
...
Four rounds of review, eleven issues total, all addressed rather than merely acknowledged. The first round caught outright errors (facility undercount, a scenario-definition bug). The second round caught claims that were individually defensible but not yet earned at the confidence level they were stated with — most held up or got stronger (matched-penalty sweep, leave-one-out, edge-overlap, workforce denominator), one did not (the chokepoint mechanism, §7). The third round caught precision failures in how the *retraction itself* was worded, plus a follow-up check (§7b) that confirmed the retraction with an independent metric and, as a side effect, surfaced a measurement caveat nobody had asked about. The fourth round didn't accept that caveat as a caveat — it correctly identified an actual bug underneath it (§7c) and asked whether the core Finding 3 comparison, not just its retracted mechanism, would survive being recomputed correctly. It did, at a corrected magnitude (94.0%→68.6%, 74.0%→51.6%, direction unchanged), but that was a genuine open question up to the moment the numbers came back, not a foregone conclusion — the three possible outcomes (survives / shrinks to nothing / reverses) were all live, and the finding would have been reported as reversed if that's what the corrected pipeline had shown.
