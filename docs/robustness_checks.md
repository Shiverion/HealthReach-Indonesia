...
**The test (Null A, as scoped in review — see the conversation record for why Null B/C, road-class-matched or spatially-constrained nulls, were explicitly deferred rather than attempted):** `src/16_null_model_random_edges.py` removes 55,983 edges chosen **uniformly at random** from the full 562,990-edge network — same count as the real Sentinel-1 flood — and measures the resulting largest connected component. Repeated 200 times. If real flood damage reflects targeting of structurally important edges better than chance, the observed largest-component-percentage should sit at the *low* (more damaged) end of this random distribution.
...
**All 200 of 200 random trials caused more damage than the actual flood.** Empirical randomization p = (200+1)/(200+1) = 1.0000, using the correct Monte Carlo estimator for this null. This is unambiguous, not a borderline result: even the *least* damaging random trial (4.47%) still fragmented the network more than the real flood pattern did (9.06%).

**What this means, stated plainly:** the "real floods hit chokepoints, which is why fewer affected edges still cause more damage than the proxy" mechanism, as originally proposed, is refuted by this test — not weakly, not ambiguously, refuted. If anything, the direction is reversed: a real, spatially-contiguous flood is *less* damaging to overall network connectivity than the same number of edges failing independently at random, almost certainly because random failures are scattered across the whole province (each one able to sever an unrelated, statistically independent local branch), while a real flood's damage is geographically concentrated in one region, leaving the rest of the province's internal connectivity untouched relative to itself.
...
Two rounds of review, eight issues total, all addressed rather than merely acknowledged. The first round caught outright errors (facility undercount, a scenario-definition bug). The second round caught something harder — claims that were individually defensible but not yet earned at the confidence level they were stated with. Most of those checks left the underlying findings intact or stronger: the matched-penalty sweep turned a qualitative proxy-vs-observed comparison into a controlled, quantitative one; leave-one-out, the edge-overlap rule, and the workforce denominator check all held within an honestly-reported margin. One did not: the null model in §7 directly refuted the proposed chokepoint mechanism, and that finding was downgraded rather than rescued. Three solid results and one retracted hypothesis, stated plainly, is a better account of this project than four solid results would have been if one of them hadn't actually earned it.
