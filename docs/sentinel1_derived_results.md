...
**Confirmed independently (Null A2, `docs/robustness_checks.md` §7b):** largest-connected-component size isn't the same phenomenon as population losing healthcare access specifically, so a follow-up check reran the identical randomization using population-disconnected-from-any-facility as the metric instead. It agreed with Null A rather than diverging from it: random removal disconnects more population from facilities than the real flood by this metric too (mean 89.5% vs. observed 68.65%, empirical p≈0.005). Building that check carefully also surfaced a real caveat in the headline pipeline: it restricts pathfinding to the single largest post-disruption fragment, so population stranded in a smaller fragment with its own local facility is counted as "no access" — which is why Null A2's observed figure (68.65%) differs from the "94.0%" reported above for the same real scenario. Both are computed correctly; they answer different questions, and the headline figure likely overstates disconnection in heavily-fragmented scenarios. Full explanation in the linked section.
