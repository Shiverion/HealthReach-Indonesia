...
**Status:** Complete, and corrected after four rounds of review — see `docs/robustness_checks.md` for the fixes applied (facility completeness 90→365, proxy-moderate scenario definition, a matched-penalty sweep, a null model that retracted the original chokepoint-mechanism claim (§7), and a severe-pipeline pathing bug found and fixed (§7c)) and why they mattered. This document reflects the corrected numbers and the retraction; it supersedes the first-draft figures that circulated briefly before review.
...
| Sentinel-1 severe (impassable, stress test) | 31.4% | 28.0% | 30.4% | 31.2% |
...
The severe scenario disconnects more of the population (**68.6%**, corrected — see the pathing-bug fix below) than the equivalent proxy-based severe scenario (**51.6%**, corrected) despite affecting fewer roads (9.94% of edges vs. 17.3%). **This comparison is a fact.** It is unchanged by the facility-completeness fix, since it depends only on road-network topology, not on facility count.

**A pathing bug was found and fixed in this pipeline.** The severe scenario's travel-time computation originally restricted Dijkstra to the single largest post-disruption connected component, silently counting population stranded in a smaller fragment as disconnected even when that fragment retained its own local facility. Fixed (`src/06_flood_disruption.py`, `src/11_flood_disruption_sentinel1.py`): the original, buggy figures were 94.0% (Sentinel-1) and 74.0% (proxy) — both dropped substantially once corrected, and the corrected Sentinel-1 figure (68.6%) matches Null A2's independently-computed 68.65% almost exactly, confirming the fix. **The comparison direction survives the correction:** 68.6% > 51.6%. Full account: `docs/robustness_checks.md` §7c.
...
**Confirmed independently (Null A2, `docs/robustness_checks.md` §7b):** largest-connected-component size isn't the same phenomenon as population losing healthcare access specifically, so a follow-up check reran the identical randomization using population-disconnected-from-any-facility as the metric instead. It agreed with Null A rather than diverging from it: random removal disconnects more population from facilities than the real flood by this metric too (mean 89.5% vs. observed 68.65%, empirical p≈0.005). Building that check carefully also surfaced a real bug in the headline pipeline (it restricted pathfinding to the single largest post-disruption fragment) — which is why Null A2's figure (68.65%) originally differed from the pipeline's reported "94.0%" for the same real scenario. The bug is now fixed (§7c); the pipeline reports 68.6% for this scenario too, matching Null A2 almost exactly.
