...
| Sentinel-1 — severe (stress test) | 31.4% | 28.0% | 30.4% | 31.2% |
...
The observed-extent severe scenario disconnects more of the population (68.6%) than the proxy-based severe scenario (51.6%) despite affecting fewer roads (9.94% of edges vs. 17.3%). **This specific comparison is a fact, corrected once (see the note below) and unaffected by what follows in this section.**

**A pathing bug in the severe pipeline, found and fixed, not just noted as a limitation.** The severe scenario's travel-time computation originally restricted Dijkstra to the single largest post-disruption connected component, silently counting any population stranded in a smaller fragment as having no facility access even when that fragment contained its own working facility. `networkx.multi_source_dijkstra_path_length` already handles disconnected graphs correctly on its own — a node unreachable from every source is simply absent from the result, and a node sharing *any* component with a facility gets a correct distance, regardless of whether that's the largest component. Removing the restriction (`src/06_flood_disruption.py`, `src/11_flood_disruption_sentinel1.py`) corrected the severe-scenario figures substantially: proxy-severe disconnection dropped from a buggy 74.0% to **51.6%**, and Sentinel-1-severe from a buggy 94.0% to **68.6%** — the latter matching Null A2's independently-computed 68.65% almost exactly, which is itself a strong internal consistency check that the fix is correct. **The comparison direction survives:** 68.6% > 51.6%, so the core empirical claim — observed extent disconnects more population than the proxy despite affecting far fewer roads — holds at the corrected, less extreme magnitude. Full account: `docs/robustness_checks.md` §7b.
...
- **A pathing bug in the severe-scenario pipeline was found and fixed during this review, not left as a caveat.** Discovered while building the Null A2 check (`docs/robustness_checks.md` §7b): the severe pipeline originally restricted Dijkstra to the single *largest* post-disruption component, so population stranded in a smaller fragment that retained its own local facility was miscounted as disconnected. Fixed in `src/06`/`src/11` (§4.4 has the corrected figures); all severe-scenario numbers in this manuscript reflect the fix. The moderate scenarios were never affected — a pure-penalty scenario never fragments the graph, so `G_scenario` was already a single component for every moderate run, meaning the baseline, both moderate scenarios, the matched-penalty sweep, leave-one-out, and the workforce-denominator check were all unaffected by this bug from the start.
