...
**Status:** Analysis complete, including four rounds of review that caught and fixed several substantive issues — a facility undercount, a disruption-scenario bug, a randomization null model that strongly contradicted an early "chokepoint mechanism" claim (reported as a retraction rather than reframed), and a severe-pipeline pathing bug found while building a follow-up null model and fixed before the results were frozen. See `docs/robustness_checks.md`. Primary write-up: `docs/manuscript.md`. Current numbers: `docs/sentinel1_derived_results.md`. `docs/phase1_summary.md` and `docs/phase2_summary.md` are kept for methodological transparency (earlier interim results, both from the BNPB hazard-zone proxy and pre-review-fix).
